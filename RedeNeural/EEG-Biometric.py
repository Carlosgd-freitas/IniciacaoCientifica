...
train = [4, 12]                # Tasks used for training and validation
test = [8]                     # Tasks used for testing
window_size = 1920
offset = 200
...
def load_data(folder_path, train_tasks, test_tasks, verbose=0):
    ...
    for train_task in train_tasks:
        for i in range(1, num_classes + 1):
            train_content = read_EDF(folder_path+'S{:03d}/S{:03d}R{:02d}.edf'.format(i, i, train_task), occipital_lobe_yang)
    ...
    for test_task in test_tasks:
        for i in range(1, num_classes + 1):
            test_content = read_EDF(folder_path+'S{:03d}/S{:03d}R{:02d}.edf'.format(i, i, test_task), occipital_lobe_yang)
